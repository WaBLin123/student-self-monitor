...
- 下課節次已具名：`第一節下課`、`第二節下課`、`第三節下課`、`第五節下課`
- 顯示模式：
  - `當天`：自動判斷今天星期幾，只顯示當日
  - `當週`：顯示整週
...
- 同步 payload 每筆資料都帶 `forceSync: true`，用於要求後端保留完整節次（即使空值）
- 觸發操作包含：
  - 簽到 / 簽退 / 服藥
  - checkbox 勾選變更
  - 按 `✔` 儲存教務處檢核
...
- 依 `recordId` upsert（存在則更新，不存在則新增）
- 若前端帶 `forceSync: true`，請勿過濾該列（即使欄位為空）
...
- 未提供歷史查詢頁與圖表儀表板
- Google Sheet 是否保留空值列，取決於 Apps Script `doPost` 的過濾邏輯
